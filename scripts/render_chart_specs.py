...
FACE_COLOR = "#FBFCFE"
AXIS_COLOR = "#BCC7D6"
POSITIVE_COLOR = "#3B7A57"
NEGATIVE_COLOR = "#B85757"
...
def apply_academic_style(fig: plt.Figure, ax: plt.Axes, spec: ChartSpec) -> None:
    ...
    ax.grid(axis="y", color=GRID_COLOR, linestyle="--", linewidth=0.8, alpha=0.9)
    ...
    fig.suptitle(spec.title, x=0.065, y=0.965, ha="left", va="top", fontsize=18, fontweight="bold", color=NEUTRAL_TEXT)
    if spec.summary:
        fig.text(0.065, 0.915, spec.summary, ha="left", va="top", fontsize=10.5, color=SUBTLE_TEXT)
...
def render_bar(spec: ChartSpec, output_path: Path, dpi: int) -> None:
    ...
    plt.tight_layout(rect=(0.04, 0.07, 0.98, 0.9))
...
def render_combo_bar_line(spec: ChartSpec, output_path: Path, dpi: int) -> None:
    ...
    ax.legend(handles1 + handles2, labels1 + labels2, frameon=False, loc="upper right", fontsize=10)
...
def render_waterfall(spec: ChartSpec, output_path: Path, dpi: int) -> None:
    ...
    for idx, (label, value) in enumerate(zip(labels, values)):
        ...
        bar = ax.bar(x[idx], height, bottom=bottom, color=color, width=0.62, edgecolor="white", linewidth=0.8)
...
def render_pie(spec: ChartSpec, output_path: Path, dpi: int) -> None:
    ...
    fig.suptitle(spec.title, x=0.08, y=0.965, ha="left", va="top", fontsize=18, fontweight="bold", color=NEUTRAL_TEXT)
    if spec.summary:
        fig.text(0.08, 0.915, spec.summary, ha="left", va="top", fontsize=10.5, color=SUBTLE_TEXT)
    ...
    ax.legend(wedges, legend_labels, frameon=False, bbox_to_anchor=(1.02, 0.5), loc="center left", fontsize=10)
    ...
    plt.tight_layout(rect=(0.04, 0.07, 0.92, 0.9))
    fig.savefig(output_path, dpi=dpi, facecolor=fig.get_facecolor(), bbox_inches="tight")
    plt.close(fig)
