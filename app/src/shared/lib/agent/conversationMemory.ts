...
const MAX_ARTIFACT_PAYLOAD = 12000
...
function shouldCaptureArtifact(toolCall: ToolCallRecord): boolean {
  ...
  return toolCall.result.length > 600 || ['query_with_hierarchy', 'query_biz_data', 'query_monthly_plan', 'resolve_org_nodes'].includes(toolCall.name)
}
