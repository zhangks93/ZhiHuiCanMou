...
export function buildFinancialAnalysisRuntimeContextBlock(
  ...
  return [
    ...
    '- guidance: 只能使用上下文列出的合法期间；累计期间不能自行推导不存在的值。完整报告优先使用报告包和写作素材，表格优先使用目标对标宽表、学年目标表、两层组织表和成本费用宽表。不得编造应收、资金计划或专项费用明细，只能在结尾说明补数限制。轻量查数和非报告分析优先使用层级查询结果，直接分析返回的组织上下级关系。分析中必须使用返回的实际值、目标值、完成率、差额、同比，并在写作前计算可推导的汇总、占比、环比、同比和贡献。完整报告必须区分当月、截至当月累计和学年目标累计；当月实际值可合并，累计实际值必须按学年预算和突围考核分别展示。完整报告必须至少下钻两层组织并给出具体数字，有数据的下钻单位尽量呈现。终稿的标题、正文、表头、预警和说明只能使用中文，不得出现英文、内部字段名、工具字段名、数据表名、内部口径枚举、叶子节点等非业务表达。',
  ].join('\n')
}
