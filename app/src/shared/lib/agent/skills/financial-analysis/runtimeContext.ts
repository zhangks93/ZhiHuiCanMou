...
export function buildFinancialAnalysisRuntimeContextBlock(
  ...
  return [
    ...
    '- data_unit: 金额类经营指标默认以万元保存和返回；率类指标按百分比含义展示，人效/倍数类指标按工具返回含义展示。除非用户明确要求，不做元/万元换算。',
    ...
    '- guidance: 只能使用上下文列出的合法期间；累计期间不能自行推导不存在的值。优先使用层级查询结果，直接分析返回的组织上下级关系。分析中必须使用返回的实际值、目标值、完成率、差额、同比，并在写作前计算可推导的汇总、占比、环比、同比和贡献。不得编造应收、资金计划或专项费用明细，只能作为待补数据或下一步专项分析提出。输出只能使用中文业务表达，不得出现英文、内部字段名、工具字段名、数据表名、内部口径枚举、叶子节点等非业务表达。',
  ].join('\n')
}
