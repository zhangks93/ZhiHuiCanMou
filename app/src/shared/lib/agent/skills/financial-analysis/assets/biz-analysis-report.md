# 后勤集团经营分析报告模板

**汇报单位**：`query_business_report_pack.metadata.scope_name`  
**统计周期**：当月 `metadata.month`；截至当月累计 `metadata.cumulative_to_month_period`；学年目标累计 `metadata.school_year_target_period`  
**数据单位**：万元  
**数据来源**：系统经营数据和组织层级数据

> 模板是写作框架，可按组织范围和数据完整度合并小节。数据选择和硬约束以报告生成规范和质量契约为准。
> 终稿以中文为主，可保留必要业务缩写或专有英文表达；不得出现内部字段名、工具名、数据表名或枚举值；所有内部状态必须写成中文，如“达标”“关注”“风险”“缺数”。

## 1. 经营摘要与学年目标判断

用 3-6 条要点写清：
- 收入和税前利润是否达成目标。
- 收入完成与利润完成是否匹配。
- 主要增长点、拖累点和风险点。
- 当前状态对累计目标或后续月份的影响。

本节使用 `school_year_goal_assessment_table`，只覆盖营业收入和税前利润。

建议表头：

| 指标 | 实际值 | 学年进度 | 学年预算目标 | 学年预算完成率 | 学年预算达成概率 | 学年预算风险 | 突围考核目标 | 突围考核完成率 | 突围考核达成概率 | 突围考核风险 |
|---|---:|---:|---:|---:|---|---|---:|---:|---|---|

## 2. 目标对标与实际完成

优先使用目标对标宽表。当月实际值只展示一次，并列展示学年预算与突围考核目标、完成率和差额；截至当月累计和学年目标累计必须分别展示学年预算实际值与突围考核实际值，不能用同一个实际值计算两个口径的完成率。

建议表头：

| 期间 | 指标 | 实际值 | 学年预算实际值 | 突围考核实际值 | 同比 | 环比 | 学年预算目标 | 学年预算完成率 | 学年预算差额 | 突围考核目标 | 突围考核完成率 | 突围考核差额 |
|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|

表后说明收入缺口、利润缺口、利润转化是否匹配，以及黄色 / 红色预警。

## 3. 组织结构、贡献与拖累

按 `scope_profile.recommended_report_focus` 调整颗粒度：
- 集团：区域 / 中心结构、贡献排行、缺口排行和结构质量。
- 区域 / 中心：直接下级差异、重点单元拖累和费用压力。
- 明细单位 / 项目：自身目标达成、趋势和费用风险。

必须同时使用直接下级、第二层单位、重点下钻单位、项目异常、差异排行和重点单位卡片中的数字证据。若提问组织存在二级下属，正文尽量呈现有数据的第二层单位，并写出收入、税前利润、人力成本或毛利率等具体数字。

## 4. 成本费用与效率

优先使用成本费用宽表、成本费用汇总和费用超预算排行。

重点覆盖：人力成本及明细、餐饮支出、物资销售成本、其他支出、营业外支出、车辆费用、能耗费、差旅费、业务招待费、人效类指标。

## 5. 风险判断与后续动作

使用风险预警、重点单位风险和写作素材中的动作要点输出动作表。风险等级必须写中文，不得直接输出内部英文状态值。

建议表头：

| 关注事项 | 对象 | 证据 | 风险等级 | 后续动作 |
|---|---|---|---|---|

动作必须对应具体对象和证据，避免只写“加强管理、持续关注”。

## 6. 数据限制与待补说明

仅当报告包显示缺数时输出。

写 2-5 条短说明：缺失字段、原因、业务侧需补充内容、补数后需复核事项。
