...
describe('biz data org scope identity', () => {
  it('builds distinct scope keys for duplicate node names under different paths', () => {
    const first = node('第一食堂', '第二区域', '餐饮中心')
    const second = node('第一食堂', '第三区域', '餐饮中心')

    expect(buildOrgScopeKey(first)).toBe('智汇后勤集团 / 第二区域 / 餐饮中心 / 第一食堂')
    expect(buildOrgScopeKey(second)).toBe('智汇后勤集团 / 第三区域 / 餐饮中心 / 第一食堂')
  ...
  it('deduplicates repeated path names for synthetic hierarchy nodes', () => {
    const synthetic = node('餐饮中心', '第二区域', '餐饮中心')

    expect(buildOrgPath(synthetic)).toEqual(['智汇后勤集团', '第二区域', '餐饮中心'])
    expect(buildOrgScopeKey(synthetic)).toBe('智汇后勤集团 / 第二区域 / 餐饮中心')
  })
})
